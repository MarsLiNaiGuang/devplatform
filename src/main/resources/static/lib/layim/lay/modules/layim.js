...
layui.define(['layer', 'laytpl', 'upload'], function(exports){
  ...
  var currentUserAvatar = window.localStorage.getItem('currentUserAvatar');
  ...
  var elemChatTpl = ['<div class="layim-chat layim-chat-{{d.data.type}}{{d.first ? " layui-show" : ""}}">'
    ...
    ,'<div class="layim-chat-footer">'
      ,'<div class="layui-unselect layim-chat-tool" data-json="{{encodeURIComponent(JSON.stringify(d.data))}}">'
        ...
        ,'<span class="layui-icon layim-tool-image" title="发送文件" layim-event="rsImage" data-type="file">&#xe61d;<input type="file" name="uploadfile"></span>'
         ,'{{# }; }}'
  ...
  var banRightMenu = function(){
    ...
    layimMain.find('.layim-list-group').on('contextmenu', 'li', function(e){
      var othis = $(this);
      var html = '<ul data-id="'+ othis[0].id +'" data-index="'+ othis.data('index') +'"><li layim-event="menuGroup" data-type="add">新增群组</li><li layim-event="menuGroup" data-type="edit">编辑群组</li><li layim-event="menuGroup" data-type="delete">删除群组</li></ul>';
  ...
  var anim = 'layui-anim-upbit', events = {
    ...
    ,rsGroupMembers: function(othis, e){
      var icon = othis.find('.layui-icon'), hide = function(){
        icon.html('&#xe61a;');
        othis.data('down', null);
        layer.close(events.rsGroupMembers.index);
      }, stopmp = function(e){stope(e)};
      ...
      if(othis.data('down')){
        ...
        icon.html('&#xe619;');
        othis.data('down', true);
        events.rsGroupMembers.index = layer.tips('<ul class="layim-members-list"></ul>', othis, {
          tips: 3
          ,time: 0
          ,anim: 5
          ,fixed: true
          ...
          ,success: function(layero){
            ...
            ,listNone = layimChat.find('.layim-chat-list').css('display') === 'none';
            var temp = {};
            temp = JSON.parse(JSON.stringify(members));
            if(hasFull){
              ul.css({
                width: $(window).width() - 22 - (listNone || 200)
              });
            ...
            temp.url = members.url + '/' + thatChat.data.id;
            post(temp, function(res){
              layui.each(res, function(index, item){
                ...
                $.each(cache.friend,function(cindex,citem){
                  $.each(citem.list,function(gindex,gitem){
                    if(gitem.id === item.id){
                      if(gitem.avatar.indexOf('avatar')!==-1){
                        item.avatar = window.rsContextPath + gitem.avatar;
                      }else{
                        item.avatar = 'data:image/jpeg;base64,' + gitem.avatar;
                      }
              ...
              //获取群员
              othis.find('.layim-chat-members').html(res.length + '人');
    ...
    ,rsImage: function(othis){
      var api = {
        file: 'uploadFile'
      },
      fileName = othis.find('input')[0]
      ,thatChat = thisChat(), upload = cache.base[api['file']] || {};
      layui.upload({
        url: upload.url || ''
        ,method: upload.type
        ,elem: othis.find('input')[0]
        ...
        ,success: function(res){
          if(res.id){
            res.data = res.data || {};
            res.data.src = window.rsContextPath+'/im/file/download/' + res.id;
            // focusInsert(thatChat.textarea[0], '<a class="file" href="'+window.rsContextPath+'/im/file/download/' + res.id + '" target="_self"></a>');
            focusInsert(thatChat.textarea[0], 'file('+ (res.data.src||'') +')['+ (res.data.name||'下载文件') +']');
            sendMessage('F');
          } else {
            layer.msg(res.msg||'上传失败');
