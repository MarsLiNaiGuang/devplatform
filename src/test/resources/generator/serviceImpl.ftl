...
import com.rs.devplatform.common.Constants;
import com.rs.devplatform.conf.framework.RsGeneralMapper;
...
public class ${beanName}ServiceImpl implements ${beanName}Service {
	...
	public boolean add(${beanName} ${beanVar}){
		if(!${beanVar}.getClass().getSimpleName().equals(${beanName}.class.getSimpleName())){//VO:
			${beanName} realEntity = new ${beanName}();
			try {
				BeanUtils.copyProperties(${beanVar}, realEntity);
				generalMapper.insert(realEntity);
			} catch (Exception e) {
				e.printStackTrace();
				return false;
			}
	...
	public boolean update(${beanName} ${beanVar}){
		...
		if(!${beanVar}.getClass().getSimpleName().equals(${beanName}.class.getSimpleName())){//VO:
			...
			try {
				BeanUtils.copyProperties(${beanVar}, realEntity);
				effRows = generalMapper.updateSelective(realEntity, entityWhere);
			} catch (Exception e) {
				e.printStackTrace();
				return false;
			}
		}else{
			effRows = generalMapper.updateSelective(${beanVar}, entityWhere);
		}
